...
export const TooltipContent = () => {
  ...
  const filteredTooltips = tooltipContent.filter((tooltip) =>
    tooltip.name.toLowerCase().includes(search.toLowerCase())
  );
  return (
    <>
      <Header onSearch={handleSearch} />

      <motion.div className="grid border-t w-screen relative dark:border-white/10 min-h-screen mt-20 place-items-start  sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4   ">
        ...
        {filteredTooltips.map((tooltip, index) => (
          <motion.div
            key={index}
            className="max-w-xl mx-auto justify-center flex"
          >
            <div
            ...
            {openModals[index] && (
              <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/50">
                ...
                >
                  ...
                  <CodeBlock code={tooltip.css} fileName={tooltip.name} />
                </div>
              </div>
            )}
          </motion.div>
